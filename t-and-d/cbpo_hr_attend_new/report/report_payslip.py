...
class payslip_details_report2(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(payslip_details_report2, self).__init__(cr, uid, name, context)
        self.localcontext.update({
            'get_details_by_rule_category2': self.get_details_by_rule_category2,
            'get_lines_by_contribution_register2': self.get_lines_by_contribution_register2,
        })

    def  get_details_by_rule_category2(self, obj):
            ...
            for key, value in result.iteritems():
                ...
                for parent in parents:
                    res.append({
                        ...
                        'code': parent.code,
                        'level': level,
                        'total': category_total, #*currency_rate,
                    })
                    level += 1
                for line in payslip_line.browse(self.cr, self.uid, value):
                    res.append({
                        ...
                        'name': line.name,
                        'code': line.code,
                        'total': line.total, #*currency_rate,
                        'level': level
                    })
        return res

    def get_lines_by_contribution_register2(self, obj):
        payslip_line = self.pool.get('hr.payslip.line')
        result = {}
        res = []
...
class wrapped_report_payslipdetails2(osv.AbstractModel):
    _name = 'report.cbpo_hr_attend_new.report_payslipdetails2'
    # _inherit = 'report.hr_payroll.report_payslipdetails'
    _inherit = 'report.abstract_report'
    # _template = 'hr_payroll.report_payslipdetails2'
    _template = 'cbpo_hr_attend_new.report_payslipdetails2'
    _wrapped_report_class = payslip_details_report2
